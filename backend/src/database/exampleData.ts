...
import { queryDeleteAllCartItems } from "./queries/cartQueries";
import { queryCreateProduct, queryDeleteAllProducts } from "./queries/productQueries";
import { queryCreateReview, queryDeleteALLReviews } from "./queries/reviewQueries";
import { queryCreateUser, queryDeleteAllUsers } from "./queries/userQueries";
...
export async function initialize() {
    ...
    await queryDeleteAllCartItems();
    log(0,'INITIALIZE','deleted all carts');
    await sleep(2000);
    await queryDeleteALLReviews();
    log(0,'INITIALIZE','deleted all reviews');
    await sleep(2000);
    await queryDeleteAllProducts();
    log(0,'INITIALIZE','deleted all products');
    await sleep(2000);
    await queryDeleteAllUsers();
    log(0,'INITIALIZE','deleted all users');

    for (let index = 0; index < users.length; index++) {
        const user = users[index];
        await queryCreateUser(user)
        await sleep(1000);
    }
    
    for (let index = 0; index < products.length; index++) {
        await queryCreateProduct(products[index]);
        await sleep(1000);
    }

    for (let index = 0; index < reviews.length; index++) {
        await queryCreateReview(reviews[index]);
        await sleep(1000);
    }
